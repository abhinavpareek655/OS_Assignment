...
void* transfer1_to_2(void* arg) {
    ...
    pthread_mutex_lock(&lock1); // Lock account1
    printf("Thread 1: Locked account1. Waiting for account2...\n");
    sleep(1); // Simulate delay to force deadlock
    pthread_mutex_lock(&lock2); // Try to lock account2 (will wait forever)
...
void* transfer2_to_1(void* arg) {
    ...
    pthread_mutex_lock(&lock2); // Lock account2
    printf("Thread 2: Locked account2. Waiting for account1...\n");
    sleep(1); // Simulate delay to force deadlock
    pthread_mutex_lock(&lock1); // Try to lock account1 (will wait forever)
